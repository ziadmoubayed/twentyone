package com.github.ziadmoubayed.twentyone.engine.feedback;

public class Messages {

    /**
     *
     */
    private Messages() {}

    public static final String GREETING = "Twenty One";
    public static final String PLAYER_NAMES = "Type the players' names separated by semi column";
    public static final String PLAYER_TURN = "Player %s's turn. You have %d points. Choose %s";
    public static final String START_GAME = "Starting %s";
    public static final String NOTIFY_BUST = "You are out";
    public static final String NOTIFY_CHOOSE_POINTS = "What which point you want to use: %s";
    public static final String NOTIFY_CARD_POINTS = "%s got %s and now has score %s";
    public static final String NOTIFY_BANK_TURN = "The Bank will play now";
    public static final String NOTIFY_GAME_ENDED = "%s Ended.";
    public static final String NOTIFY_WINNER_LOSER = "%s won against %s";
    public static final String NOTIFY_CARD_SPLIT = "%s split pair of %s";
}
